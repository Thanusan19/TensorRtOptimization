...
ROOT = '/home/localadmin/Documents/GRU/model_ckpt'
PATH = ROOT+'/Best_1S'
...
class ModelData(object):
    MODEL_FILE = "modelGRU.uff" #"model.uff"
    INPUT_NAME ="input"
...
def main():
	...
	#---------------------------------#
	#uff_model = uff.from_tensorflow(sess.graph, ["output/BiasAdd:0"])
	#uff_model = uff.from_tensorflow_frozen_model(ROOT+'/frozen_model.pb',input_nodes=['input','hidden_state'],output_nodes=['output/BiasAdd'],output_filename = ModelData.MODEL_FILE)
	print("yes")
	#----------------------------------#
	...
	# Create a builder
	builder = tf.saved_model.builder.SavedModelBuilder('./SavedModel1/')

	with tf.Session() as sess:
	    saver = tf.train.import_meta_graph(PATH+'.meta')
	    saver.restore(sess, PATH)
	    sess.run('output/BiasAdd:0',feed_dict={'input:0':X,'hidden_state:0':HS})
	...
	uff_model = uff.from_tensorflow_frozen_model('/home/localadmin/NNOptimization/TensorRtOptimization/SavedModel1/saved_model.pb',['output/BiasAdd:0'],output_filename = ModelData.MODEL_FILE)
